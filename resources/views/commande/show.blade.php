...
<commande-show
    :articles_prop="{{ $id_articles }}"
    :commande_prop="{{ $commande }}"
    :products_prop="{{ $products }}"
    :templates_prop="{{ $templates }}"
    :commandes_prop="{{ $commandes }}"
    inline-template>
    <section>
        ...
        <main class="tw-flex tw-flex-col tw-justify-center tw-items-center tw-w-screen tw-bg-gray-400 ">
            ...
            <div class="tw-flex tw-justify-center tw-bg-gray-200 tw-w-full tw-py-20">
                ...
                <a  :href=" '/commande/' + commande.id + '/export'  " class="tw-btn tw-bg-gray-900 tw-text-white tw-leading-none tw-ml-5" >
                    Télécharger .xlsx
                    <i class="fas fa-spinner fa-spin" v-if="isLoading.majStock"></i>
                </a>
                <button class="tw-btn tw-bg-gray-900 tw-text-white tw-leading-none tw-ml-5" data-toggle="modal" data-target="#ajouterNonDispo">Ajouter Non Disponible</button>
            </div>
            ...
            <div class="modal fade" id="ajouterNonDispo" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true" @keydown.enter="removeSection()">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <select class="form-control" v-model="nonDispo.commande">
                                    <option v-for="cmd in commandes" :value="cmd.id">@{{ cmd.name }}</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="">Section</label>
                                <select class="form-control" v-model="nonDispo.section">
                                    <option v-for="sect in commande.sections" :value="sect.id">@{{ sect.nom }}</option>
                                </select>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="tw-btn btn-secondary" data-dismiss="modal">Fermer</button>
                            <button type="button" class="tw-btn btn-primary" @click="ajouterNonDispo()">Ajouter Non Disponible</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="accordianId" role="tablist" aria-multiselectable="true" class="tw-mt-20 tw-w-3/4">
                <div class="card" v-for="section in commande.sections">
                    ...
                    <div  class="tw-border tw-border-gray-400 tw-shadow tw-cursor-pointer card-header tw-flex tw-justify-between tw-items-center tw-bg-gray-200 tw-text-gray-800" data-toggle="collapse" data-parent="#accordianId" :href="'#section' + section.id " aria-expanded="true" aria-controls="section1ContentId">
                        ...
                        <div class="tw-w-1/5">
                            <span class="tw-w-1/2" v-if="section.products.length > 0">@{{ numberOfElementsOrdered(section) }} Commandés / @{{ section.products.length  }} Produits</span>
                            <span class="tw-w-1/2 tw-ml-5" v-if="section.articles.length > 0">@{{ numberOfElementsOrdered(section) }} Commandés / @{{ section.articles.length  }} Nouveaux Produits</span>
                        </div>
                        <div>
                        ...
                        <div class="card-body tw-flex-col tw-items-center tw-justify-center tw-p-0">
                            ...
                            <div class="tw-flex-col tw-justify-center tw-items-center tw-bg-gray-600 tw-w-full tw-p-16">
                                ...
                                <div class="tw-w-full tw-mr-4 tw-mt-3 tw-flex tw-justify-center tw-items-center">
                                    ...
                                    <button class="ml-5 tw-btn tw-btn-dark tw-leading-none" @click="addProductToSection(section.id)">Ajouter Produit</button>
                                </div>
                                    ...
                                    >
                                        <tr v-for="(sectionnable) in section.sectionnables" v-if="sectionnable.sectionnable_type === 'App\\Product' " :key="sectionnable.id" >
                                            ...
                                            <td>
                                                <div class="form-group">
                                                    <input
                                                        type="text" class="form-control" v-model.number="sectionnable.quantite"  @input="saveQuantity(section, sectionnable)">
                                                    <p :class="sectionnable.color" v-if="sectionnable.message">@{{ sectionnable.message }}</p>
                                                </div>
                                            </td>
                                            <td>
                                                <a target="_blank" :href="'/commande/' + commande.id + '/bons-commandes/' + bc.id " v-for="bc in sectionnable.bon_commande">
                                                    <i class="fas fa-info tw-text-green-500 tw-cursor-pointer tw-mr-5"></i>
                                                </a>
                                                {{-- <i class="fas fa-edit tw-text-red-500 tw-cursor-pointer" @click="removeProduct(section, sectionnable, 'Product')"></i> --}}
                                                <i class="fas fa-trash tw-text-red-500 tw-cursor-pointer" @click="removeProduct(section, sectionnable, 'Product')"></i>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                                    ...
                                    >
                                        <tr v-for="sectionnable in section.sectionnables" :key="sectionnable.id" v-if="sectionnable.sectionnable_type === 'App\\Article' ">
                                            <td scope="row">
                                                ...
                                                <span v-if="sectionnable.article && sectionnable.article.fiche_renseignement && sectionnable.article.fiche_renseignement.moteur"> @{{sectionnable.article.fiche_renseignement.moteur.nom}} )</span>
                                            </td>
                                            <td>
                                                <div class="form-group">
                                                    <input type="text" class="form-control" v-model.number="sectionnable.quantite" placeholder="" @input="saveQuantity(section, sectionnable)">
                                                </div>
                                            </td>
                                            <td>
                                                <a target="_blank" :href="'/commande/' + commande.id + '/bons-commandes/' + bc.id "
                                                    v-for="bc in sectionnable.bon_commande">
                                                    <i class="fas fa-info tw-text-green-500 tw-cursor-pointer tw-mr-5"></i>
                                                </a>
                                                <i class="fas fa-trash tw-text-red-500 tw-cursor-pointer" @click="removeProduct(section, sectionnable, 'Article')"></i>
                                                <p :class="sectionnable.article.color" v-if="sectionnable.article && sectionnable.article.message">@{{ sectionnable.article.message.text }}</p>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
