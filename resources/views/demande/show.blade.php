            ...
            <tbody>
                ...
                <template v-for="(sectionnable, index) in demande.sectionnables" v-if="sectionnable.product">
                    ...
                    <tr class="">
                        ...
                        <td v-else v-show="toDisplay.traduction"></td>
                        {{-- Différente Offre --}}
                        ...
                        <td v-show="toDisplay.offerAed">
                            ...
                            :class="{'tw-border-red-500': sectionnable.hasError, 'focus:tw-border-red-500' : sectionnable.hasError }"
                    ...
                    <tr v-show="sectionnable.displayDetails" class=" tw-bg-gray-400 tw-text-black tw-ml-6">
                        <td colspan="10">
                            {{-- Bons Commandes --}}
                            <h3 class="tw-mt-2 tw-font-bold tw-text-xl" v-if="sectionnable.bon_commande.length > 0">
                                Bon Commande Correspondants :
                            </h3>

                            <div class="tw-mt-3 tw-mb-6" v-if="sectionnable.bon_commande" v-for="bc in sectionnable.bon_commande">
                                <a :href="'/commande/' + demande.commande_id + '/bons-commandes/' + bc.id">
                                    @{{ bc.nom }}
                                </a>
                            </div>

                            <h3 class="tw-mt-2 tw-font-bold tw-text-xl" v-if="sectionnable.demandes.length > 0">
                            ...
                            <div class="tw-mt-5" >
                                <table>
                                    ...
                                    <tbody>
                                        <tr v-if="sectionnable.demandes.length > 0" v-for="(dem, idx) in sectionnable.demandes">
                                            <td>
                                            ...
                                            <td>
                                                <span v-if="dem.pivot.differente_offre && dem.pivot.reference_differente_offre" class="tw-ml-5">@{{
                                                dem.pivot.reference_differente_offre }}</span>
                                            </td>
                                            <td>
                ...
                {{-- ARTICLES --}}
                <template
                    v-for="(sectionnable, index) in demande.sectionnables"
                    v-if="sectionnable.article"
                >
                    <tr>
                        <td>
                            <i class="fas fa-chevron-down tw-cursor-pointer" @click="toggleDetails(sectionnable)" v-if="sectionnable.displayDetails"></i>
                        ...
                        <td>
                            <i class="fas fa-trash tw-text-red-500 tw-cursor-pointer" @click="openDeleteModal(sectionnable)"></i>
                            <i class="fa fa-share-square tw-cursor-pointer tw-text-green-500 tw-ml-2" aria-hidden="true"
                                @click="openMoveModal(sectionnable, index)"></i>
                    ...
                    <tr v-show="sectionnable.displayDetails" class=" tw-bg-gray-700 tw-text-white">
                        <td colspan=10>
                            ...
                            <div
                                v-if="sectionnable.bon_commande"
                                v-for="bc in sectionnable.bon_commande"
                            >
                                <a
                                    :href="'/commande/' + demande.commande_id + '/bons-commandes/' + bc.id"
                                >
                                    @{{ bc.nom }}
                                </a>
                            </div>
